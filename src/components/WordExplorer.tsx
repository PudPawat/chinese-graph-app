import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import {
  Modal,
  Platform,
  Pressable,
  ScrollView,
  StyleSheet,
  Text,
  useWindowDimensions,
  View,
} from 'react-native';
import { Gesture, GestureDetector } from 'react-native-gesture-handler';
import Animated, {
  runOnJS,
  useAnimatedStyle,
  useSharedValue,
  withSpring,
  withTiming,
} from 'react-native-reanimated';
import { SafeAreaView } from 'react-native-safe-area-context';
import type { DirectionalNav, NavDirection, VocabularyGraph, VocabularyWord } from '../types/vocabulary';
import {
  DIRECTION_ARROWS,
  buildDirectionalNav,
  directionFromSwipe,
  getNeighborInDirection,
} from '../lib/wordNavigator';
import { HSK_COLORS } from '../lib/graphEngine';
import { truncateGloss } from '../lib/database';
import { colors } from '../constants/theme';

interface Props {
  visible: boolean;
  word: VocabularyWord | null;
  pool: VocabularyWord[];
  graph: VocabularyGraph | null;
  onClose: () => void;
  onWordChange: (word: VocabularyWord) => void;
}

interface SlideLayout {
  width: number;
  height: number;
  gapX: number;
  gapY: number;
  sideW: number;
  centerW: number;
  vertH: number;
  centerH: number;
  hanziSize: number;
  tradSize: number;
  pinyinSize: number;
  bodySize: number;
  previewCharSize: number;
  previewLabelSize: number;
}

const AnimatedView = Animated.createAnimatedComponent(View);

const SPRING = { damping: 22, stiffness: 240 };
const SLIDE_MS = 220;

function buildSlideLayout(width: number, height: number): SlideLayout {
  const gapX = width * 0.05;
  const gapY = height * 0.05;
  const sideW = width * 0.1;
  const centerW = width * 0.7;
  const vertH = height * 0.1;
  const centerH = height * 0.7;

  return {
    width,
    height,
    gapX,
    gapY,
    sideW,
    centerW,
    vertH,
    centerH,
    hanziSize: Math.min(centerW * 0.2, centerH * 0.16, 54),
    tradSize: Math.min(centerW * 0.09, 26),
    pinyinSize: Math.min(centerW * 0.065, 18),
    bodySize: Math.min(centerW * 0.048, 15),
    previewCharSize: Math.min(sideW * 0.38, vertH * 0.34, 22),
    previewLabelSize: Math.min(sideW * 0.16, vertH * 0.14, 9),
  };
}

function swipeThreshold(width: number, height: number): number {
  return Math.min(width, height) * 0.14;
}

function NavPreview({
  slot,
  direction,
  onPress,
  layout,
  style,
}: {
  slot: DirectionalNav[NavDirection];
  direction: NavDirection;
  onPress: () => void;
  layout: SlideLayout;
  style?: object;
}) {
  const arrow = DIRECTION_ARROWS[direction];
  const hskColor = slot ? HSK_COLORS[slot.word.hskLevel] ?? colors.border : colors.border;
  const isVertical = direction === 'up' || direction === 'down';

  return (
    <Pressable
      onPress={slot ? onPress : undefined}
      style={[
        styles.previewSlot,
        !slot && styles.previewEmpty,
        style,
      ]}
    >
      <Text style={[styles.previewArrow, { fontSize: layout.previewLabelSize + 5 }]}>{arrow}</Text>
      {slot ? (
        <>
          <Text
            style={[styles.previewChar, { fontSize: layout.previewCharSize }]}
            numberOfLines={1}
            adjustsFontSizeToFit
          >
            {slot.word.simplified}
          </Text>
          {isVertical ? (
            <Text
              style={[styles.previewGloss, { fontSize: layout.previewLabelSize }]}
              numberOfLines={1}
            >
              {truncateGloss(slot.word.english, 14)}
            </Text>
          ) : null}
          <Text style={[styles.previewShared, { color: hskColor, fontSize: layout.previewLabelSize }]}>
            {slot.sharedChar}
          </Text>
        </>
      ) : (
        <Text style={styles.previewEmptyText}>—</Text>
      )}
    </Pressable>
  );
}

function WordCenter({ word, layout }: { word: VocabularyWord; layout: SlideLayout }) {
  const hskColor = HSK_COLORS[word.hskLevel] ?? '#64748b';

  return (
    <View style={styles.centerCard}>
      <View style={styles.centerHeader}>
        <Text style={[styles.centerHanzi, { fontSize: layout.hanziSize }]}>{word.simplified}</Text>
        <View style={[styles.hskBadge, { backgroundColor: hskColor }]}>
          <Text style={styles.hskBadgeText}>HSK {word.hskLevel}</Text>
        </View>
      </View>
      <Text style={[styles.centerTraditional, { fontSize: layout.tradSize }]} numberOfLines={1}>
        {word.traditional}
      </Text>
      <Text style={[styles.centerPinyin, { fontSize: layout.pinyinSize }]} numberOfLines={1}>
        {word.pinyin}
      </Text>

      <ScrollView
        style={styles.centerScroll}
        contentContainerStyle={styles.centerScrollContent}
        showsVerticalScrollIndicator={false}
        bounces={false}
      >
        <View style={styles.centerRow}>
          <Text style={styles.centerLabel}>English</Text>
          <Text style={[styles.centerValue, { fontSize: layout.bodySize, lineHeight: layout.bodySize * 1.45 }]}>
            {word.english}
          </Text>
        </View>
        <View style={styles.centerRow}>
          <Text style={styles.centerLabel}>Thai</Text>
          <Text style={[styles.centerValue, { fontSize: layout.bodySize, lineHeight: layout.bodySize * 1.45 }]}>
            {word.thai || '—'}
          </Text>
        </View>
      </ScrollView>
    </View>
  );
}

export function WordExplorer({ visible, word, pool, graph, onClose, onWordChange }: Props) {
  const { width: screenW, height: screenH } = useWindowDimensions();

  const [displayWord, setDisplayWord] = useState<VocabularyWord | null>(word);
  const [areaSize, setAreaSize] = useState({ width: screenW, height: screenH * 0.82 });
  const [incomingWord, setIncomingWord] = useState<VocabularyWord | null>(null);

  const layout = useMemo(
    () => buildSlideLayout(areaSize.width, areaSize.height),
    [areaSize.width, areaSize.height],
  );

  const slideX = useSharedValue(0);
  const slideY = useSharedValue(0);
  const incomingX = useSharedValue(0);
  const incomingY = useSharedValue(0);
  const isAnimating = useSharedValue(false);

  useEffect(() => {
    if (word) setDisplayWord(word);
  }, [word]);

  const nav = useMemo(
    () => (displayWord ? buildDirectionalNav(displayWord, pool, graph) : null),
    [displayWord, pool, graph],
  );

  const resetSlides = useCallback(() => {
    slideX.value = 0;
    slideY.value = 0;
    incomingX.value = 0;
    incomingY.value = 0;
    setIncomingWord(null);
  }, [slideX, slideY, incomingX, incomingY]);

  const finishNavigation = useCallback(
    (nextWord: VocabularyWord) => {
      setDisplayWord(nextWord);
      onWordChange(nextWord);
      resetSlides();
      isAnimating.value = false;
    },
    [onWordChange, resetSlides, isAnimating],
  );

  const animateToNeighbor = useCallback(
    (dir: NavDirection) => {
      if (!displayWord || !nav || isAnimating.value) return;
      const slot = getNeighborInDirection(nav, dir);
      if (!slot) {
        slideX.value = withSpring(0, SPRING);
        slideY.value = withSpring(0, SPRING);
        return;
      }

      isAnimating.value = true;
      setIncomingWord(slot.word);

      const slideW = layout.centerW + layout.gapX;
      const slideH = layout.centerH + layout.gapY;
      const originX = slideX.value;
      const originY = slideY.value;
      const deltaX = dir === 'left' ? -slideW : dir === 'right' ? slideW : 0;
      const deltaY = dir === 'up' ? -slideH : dir === 'down' ? slideH : 0;

      incomingX.value = originX - deltaX;
      incomingY.value = originY - deltaY;

      slideX.value = withTiming(originX + deltaX, { duration: SLIDE_MS });
      slideY.value = withTiming(originY + deltaY, { duration: SLIDE_MS });
      incomingX.value = withTiming(0, { duration: SLIDE_MS });
      incomingY.value = withTiming(0, { duration: SLIDE_MS }, (finished) => {
        if (finished) {
          runOnJS(finishNavigation)(slot.word);
        }
      });
    },
    [
      displayWord,
      nav,
      layout.centerW,
      layout.gapX,
      layout.centerH,
      layout.gapY,
      slideX,
      slideY,
      incomingX,
      incomingY,
      isAnimating,
      finishNavigation,
    ],
  );

  const navigate = useCallback(
    (dir: NavDirection) => {
      animateToNeighbor(dir);
    },
    [animateToNeighbor],
  );

  const navRef = useRef(nav);
  navRef.current = nav;
  const layoutRef = useRef(layout);
  layoutRef.current = layout;

  const handlePanEnd = useCallback(
    (translationX: number, translationY: number) => {
      if (isAnimating.value) return;

      const { width, height } = layoutRef.current;
      const dir = directionFromSwipe(translationX, translationY, swipeThreshold(width, height));

      if (dir && navRef.current?.[dir]) {
        animateToNeighbor(dir);
        return;
      }

      setIncomingWord(null);
      slideX.value = withSpring(0, SPRING);
      slideY.value = withSpring(0, SPRING);
      incomingX.value = withSpring(0, SPRING);
      incomingY.value = withSpring(0, SPRING);
    },
    [animateToNeighbor, isAnimating, slideX, slideY, incomingX, incomingY],
  );

  const panStart = useRef({ x: 0, y: 0 });
  const panGesture = Gesture.Pan()
    .onBegin(() => {
      if (isAnimating.value) return;
      panStart.current = { x: slideX.value, y: slideY.value };
    })
    .onUpdate((event) => {
      if (isAnimating.value) return;
      slideX.value = panStart.current.x + event.translationX;
      slideY.value = panStart.current.y + event.translationY;
    })
    .onEnd((event) => {
      runOnJS(handlePanEnd)(event.translationX, event.translationY);
    });

  const currentStyle = useAnimatedStyle(() => ({
    transform: [{ translateX: slideX.value }, { translateY: slideY.value }],
  }));

  const incomingStyle = useAnimatedStyle(() => ({
    transform: [{ translateX: incomingX.value }, { translateY: incomingY.value }],
  }));

  useEffect(() => {
    if (!visible || Platform.OS !== 'web') return;

    const onKey = (event: KeyboardEvent) => {
      const keyMap: Record<string, NavDirection> = {
        ArrowUp: 'up',
        ArrowDown: 'down',
        ArrowLeft: 'left',
        ArrowRight: 'right',
      };
      const dir = keyMap[event.key];
      if (dir) {
        event.preventDefault();
        navigate(dir);
      }
      if (event.key === 'Escape') onClose();
    };

    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [visible, navigate, onClose]);

  useEffect(() => {
    if (!visible) resetSlides();
  }, [visible, resetSlides]);

  if (!displayWord || !nav) return null;

  return (
    <Modal visible={visible} animationType="fade" transparent={false} onRequestClose={onClose}>
      <SafeAreaView style={styles.screen} edges={['top', 'left', 'right', 'bottom']}>
        <View style={styles.topBar}>
          <Pressable onPress={onClose} style={styles.closeBtn}>
            <Text style={styles.closeBtnText}>✕</Text>
          </Pressable>
          <Text style={styles.topHint} numberOfLines={1}>
            Swipe ↑↓←→ · 70% center · 10% neighbors
          </Text>
        </View>

        <GestureDetector gesture={panGesture}>
          <View
            style={styles.slideArea}
            onLayout={(event) => {
              const { width, height } = event.nativeEvent.layout;
              setAreaSize({ width, height });
            }}
          >
            <View style={{ height: layout.vertH, marginBottom: layout.gapY, alignItems: 'center' }}>
              <NavPreview
                slot={nav.up}
                direction="up"
                onPress={() => navigate('up')}
                layout={layout}
                style={{ width: layout.centerW, height: layout.vertH }}
              />
            </View>

            <View
              style={{
                height: layout.centerH,
                marginBottom: layout.gapY,
                flexDirection: 'row',
                alignItems: 'stretch',
              }}
            >
              <NavPreview
                slot={nav.left}
                direction="left"
                onPress={() => navigate('left')}
                layout={layout}
                style={{ width: layout.sideW, height: layout.centerH, marginRight: layout.gapX }}
              />

              <View
                style={{
                  width: layout.centerW,
                  height: layout.centerH,
                  overflow: 'hidden',
                  borderRadius: 18,
                }}
              >
                {incomingWord ? (
                  <AnimatedView style={[styles.centerLayer, incomingStyle]} pointerEvents="none">
                    <WordCenter word={incomingWord} layout={layout} />
                  </AnimatedView>
                ) : null}
                <AnimatedView style={[styles.centerLayer, currentStyle]}>
                  <WordCenter word={displayWord} layout={layout} />
                </AnimatedView>
              </View>

              <NavPreview
                slot={nav.right}
                direction="right"
                onPress={() => navigate('right')}
                layout={layout}
                style={{ width: layout.sideW, height: layout.centerH, marginLeft: layout.gapX }}
              />
            </View>

            <View style={{ height: layout.vertH, alignItems: 'center' }}>
              <NavPreview
                slot={nav.down}
                direction="down"
                onPress={() => navigate('down')}
                layout={layout}
                style={{ width: layout.centerW, height: layout.vertH }}
              />
            </View>
          </View>
        </GestureDetector>
      </SafeAreaView>
    </Modal>
  );
}

const styles = StyleSheet.create({
  screen: {
    flex: 1,
    backgroundColor: colors.background,
  },
  topBar: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingHorizontal: 12,
    paddingVertical: 8,
    borderBottomWidth: 1,
    borderBottomColor: colors.border,
    backgroundColor: colors.surface,
    gap: 10,
  },
  closeBtn: {
    width: 36,
    height: 36,
    borderRadius: 18,
    backgroundColor: colors.surfaceAlt,
    alignItems: 'center',
    justifyContent: 'center',
  },
  closeBtnText: {
    color: colors.text,
    fontWeight: '700',
    fontSize: 16,
  },
  topHint: {
    color: colors.textMuted,
    fontSize: 11,
    flex: 1,
  },
  slideArea: {
    flex: 1,
    minHeight: 0,
    paddingHorizontal: '2.5%',
    paddingVertical: '2.5%',
    justifyContent: 'center',
  },
  centerLayer: {
    ...StyleSheet.absoluteFill,
  },
  previewSlot: {
    backgroundColor: colors.surface,
    borderWidth: 1.5,
    borderColor: colors.border,
    borderRadius: 14,
    paddingHorizontal: 6,
    paddingVertical: 4,
    alignItems: 'center',
    justifyContent: 'center',
    overflow: 'hidden',
  },
  previewEmpty: {
    opacity: 0.28,
  },
  previewArrow: {
    color: colors.accent,
    fontWeight: '700',
    marginBottom: 1,
  },
  previewChar: {
    color: colors.text,
    fontWeight: '800',
    textAlign: 'center',
  },
  previewGloss: {
    color: colors.textMuted,
    textAlign: 'center',
    marginTop: 1,
  },
  previewShared: {
    fontWeight: '700',
    marginTop: 1,
  },
  previewEmptyText: {
    color: colors.textMuted,
    fontSize: 14,
  },
  centerCard: {
    flex: 1,
    backgroundColor: '#1e293b',
    borderRadius: 18,
    borderWidth: 2,
    borderColor: colors.accent,
    padding: '4%',
    gap: 4,
    minHeight: 0,
    shadowColor: colors.accent,
    shadowOpacity: 0.12,
    shadowRadius: 12,
    elevation: 6,
  },
  centerHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    justifyContent: 'center',
    flexShrink: 0,
  },
  centerHanzi: {
    fontWeight: '800',
    color: colors.text,
  },
  hskBadge: {
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 8,
  },
  hskBadgeText: {
    color: colors.white,
    fontWeight: '700',
    fontSize: 10,
  },
  centerTraditional: {
    color: colors.text,
    textAlign: 'center',
    fontWeight: '600',
    flexShrink: 0,
  },
  centerPinyin: {
    color: colors.accent,
    textAlign: 'center',
    flexShrink: 0,
  },
  centerScroll: {
    flex: 1,
    minHeight: 0,
  },
  centerScrollContent: {
    gap: 6,
    paddingBottom: 2,
  },
  centerRow: {
    gap: 2,
  },
  centerLabel: {
    color: colors.textMuted,
    fontSize: 10,
    fontWeight: '700',
    textTransform: 'uppercase',
  },
  centerValue: {
    color: colors.text,
  },
});
